...
for i, j in known_values.items():
    flag_test_array[i] = chr(j)
...
print(f'm extremal indice min: {min(m.keys())} max: {max(m.keys())}')

# This is a linear system of 49 equations with 49 variables they must be equal to a vector of 49 values
# The system is represented as a numpy array
linear_system = np.zeros((49, 49), dtype=int)
vector = np.zeros(49, dtype=int)
...
while pc < len(code):
    ...
    # print(f'PC: {pc}, Opcode: {pri}, Operand: {pro}, Stack: {s}, Memory: {m}')
    ...
    match pri:
        case 0:
            ...
            pc += 12
            if column_index == -1:
                print("just got the key and I am forgetting it")
                column_index = 0
                continue
            if row_index < 49:
                linear_system[row_index, column_index] = pro
                column_index += 1
            elif row_index < 49 * 2:
                print(f'pushing value {pro} at row {row_index-49}')
                vector[row_index - 49] = pro
                row_index += 1
            elif row_index == 49 * 2 and pro == 49:
                print('Loading 49 to check the calulation')
                row_index += 1
            else:
                print(f'\n???? Loading value {pro} at row {row_index} ????', end='')
                row_index += 1
        ...
        case 1:
            ...
            pc += 2
            if len(s) == 0:
                print(f'Stack is empty at pc: {pc}, exiting...')
        ...
        case 2:
            ...
            if pro > 4095 or True:
                print(f'[{row_index%49}, {column_index}] Loading from memory at {pro} with pri {pri}, pro {pro}')
            if pro > 4280:
                pass
            s.append(m[pro])
            pc += 12
        ...
        case 3:
            # store to memory
            print(
                f'Storing value {s[-1]} at PC {pc} with pri {pri}, pro {pro} ',
            )
            # print(5 / 0)
            if pro in m:
                print(f'Overwriting memory at {pro}')
            if not s:
                raise Exception('Stack underflow')
            m[pro] = s.pop()
            pc += 12
        ...
        case 8:
            ...
            a, b = s.pop(), s.pop()
            print(
                f'Comparing {a} and {b} at PC {pc} with pri {pri}, pro {pro}',
            )
        ...
        case 13:
            ...
            # if key_doncare != key:
            #    raise Exception(
            #        f'Key mismatch: expected {key}, got {key_doncare}',
            #    )
            key = key_doncare
...
def solve_system(A, b):
    ...
    if solver.check() == sat:
        model = solver.model()
        solution = [model.evaluate(xi) for xi in x]
        return solution
    else:
        return None


solution = solve_system(linear_system, vector)
if solution:
    for i, val in enumerate(solution):
        print(f"x{i} = {val}")
    print('Flag : .;,;.{' + ''.join(chr(i) for i in solution) + '}')
else:
    print("No solution found.")

# Add the conditions we found on the bytecode translation
# flag[26] == 111
# flag[27] == 117
# flag[22] == 105
# flag[33] == 97
## Forbidden as matrix must be a square
# linear_system[49, 26] = 1
# linear_system[50, 27] = 1
# linear_system[51, 22] = 1
# linear_system[52, 33] = 1

# After execution loop
n_rows = row_index + 1  # Actual number of filled rows
# dump the matrix and the vector with numpy
linear_system.dump('linear_system.npy')
vector.dump('vector.npy')

print('Linear system:')
# print all the array not with ...
print(linear_system[:n_rows, :n_rows])
print('Vector:')
print(vector[:n_rows])


# x = solve(linear_system[:n_rows, :n_rows], vector[:n_rows])
# print('Solved linear system:')
# print(linear_system)
# print('Solved vector:')
# print(x)

x = np.linalg.solve(linear_system, vector)
x = vector

correct = np.allclose(np.dot(linear_system, x), vector)
if not correct:
